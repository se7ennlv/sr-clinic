<div class="card mb-4 border-top-primary">
    <div class="card-body">
        <h5 class="m-0 font-weight-bold text-primary pull-left">
            <i class="fa fa-th-list"></i> <strong>Summarize of Medicine Distributed</strong>
        </h5>
        <hr>
        ...
        <form name="frmDates" id="frmDates" class="form-inline" action="#" autocomplete="off" method="POST" novalidate>
            ...
            <button type="button" class="btn btn-info" id="btnShow" onclick="GetDrugDataList();"><i class="fas fa-sync-alt"></i> Process</button>
...
<script>
    ...
    function GetDrugDataList() {
        if ($('#frmDates').smkValidate()) {
            ...
            $.ajax({
                type: 'GET',
                url: "<?= site_url('ReportController/FetchMedSummary') ?>/" + fDate + '/' + tDate,
                beforeSend: function() {
                    BlockUI('Processing...');
                }
            }).done(function(data) {
